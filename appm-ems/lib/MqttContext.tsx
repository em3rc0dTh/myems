...
export const MqttProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    useEffect(() => {
        try {
            const savedData = localStorage.getItem('appm_latest_data');
            if (savedData) setLatestData(JSON.parse(savedData));
            
            const savedLogs = localStorage.getItem('appm_raw_logs');
    ...
    useEffect(() => {
        if (Object.keys(latestData).length > 0) {
            localStorage.setItem('appm_latest_data', JSON.stringify(latestData));
        }
        if (rawLogs.length > 0) {
            localStorage.setItem('appm_raw_logs', JSON.stringify(rawLogs));
        }
    }, [latestData, rawLogs]);
